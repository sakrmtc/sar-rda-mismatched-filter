...
%using optimized mismatched filter
...
Kr = 15e12;             % range modulation frequency
...
sita_r_c = (0*pi)/180;	% Beam squinte angle, 0 degrees, converted to radians here(the value of beam center crossing tims (_c_)
...
fnc = 2*Vr*sin(sita_r_c)/lamda;     % Doppler centeriod frequency, calculated according to Equation (4.33).
...
for k = 1:1                 % Generate raw echo data for k targets
    ...
    w_range = ((abs(tr_mtx-2.*R_n./c)) <= ((Tr/2).*ones(Naz,Nrg)));     % range envelope, the range window
    % =====================================================================    
...
t_ref_mtx = ones(Naz,1)*t_ref;      % matrix form
 ...
 w_ref = ones(Naz,1)*(w_ref.');      % In matrix form, each row is equally windowed.
   %C = conv2(w_ref,w_ref);
% w_ref= flattopwin(Nr);             % Distance direction, build hamming window, this is a column vector.
% w_ref = ones(Naz,1)*(w_ref.');      % In matrix form, each row is equally windowed.

%  w_ref= blackmanharris(Nr);             % Distance direction, build hamming window, this is a column vector.
%  w_ref = ones(Naz,1)*(w_ref.');      % In matrix form, each row is equally windowed.
 %q1=q(2:151,:).';
 q1=q.';
 q2=ones(Naz,1)*q1; 
...
for p = 1 : NFFT_a
    for q = 1 : N_rg   % At this time, the length of the distance direction is (Nrg-Nr+1)=N_rg        
        ...
        Rrd_fn_p_ceiled = ceil(Rrd_fn_p);        % ceil, rounded up.
        ii = ( Rrd_fn_p-(Rrd_fn_p_ceiled-R/2):-1:Rrd_fn_p-(Rrd_fn_p_ceiled+R/2-1)  );        
        rcmc_sinc = sinc(ii);
        ...
        if (Rrd_fn_p_ceiled-R/2) > N_rg    % Full right overflow
            ll = (Rrd_fn_p_ceiled-R/2-N_rg:1:Rrd_fn_p_ceiled+R/2-1-N_rg);
        else
            if (Rrd_fn_p_ceiled+R/2-1) > N_rg    % partial right overflow
                ll_1 = (Rrd_fn_p_ceiled-R/2:1:N_rg);
                ll_2 = (1:1:Rrd_fn_p_ceiled+R/2-1-N_rg);
                ll = [ll_1,ll_2];
            else
                if (Rrd_fn_p_ceiled+R/2-1) < 1    % Full left overflow (unlikely, but still something to consider)
                    ll = (Rrd_fn_p_ceiled-R/2+N_rg:1:Rrd_fn_p_ceiled+R/2-1+N_rg);
                else
                    if (Rrd_fn_p_ceiled-R/2) < 1       % partial left overflow
                        ll_1 = (Rrd_fn_p_ceiled-R/2+N_rg:1:N_rg);
                        ll_2 = (1:1:Rrd_fn_p_ceiled+R/2-1);
                        ll = [ll_1,ll_2];
                    else
                        ll = (Rrd_fn_p_ceiled-R/2:1:Rrd_fn_p_ceiled+R/2-1);
                    end                    
                end
            end
        end   
...
% 
% [row_test,column_test] = size(s_ac);      % Matrix size of s_ac_test
% [aa_test,p_test] = max(abs(s_ac));           
% [bb_test,q_test] = max(max(abs(s_ac)));
% 
% row_test_max = p_test(q_test);      % The maximum value of a two-dimensional matrix, which row is row_max.
% column_test_max = q_test;           % The maximum value of the two-dimensional matrix, in which column - column_max.
% s_ac_test_max = bb_test;            % The matrix maximum is -- x_max.
% 
% % Do row slicing, upsampling, and normalized log-magnitude plot
% s_ac_test_row_max = (s_ac(row_test_max,:));
% s_ac_test_row_max_1=s_ac_test_row_max (1,60:120);
% subplot(2,2,1)
% plot(abs(s_ac_test_row_max ))
% subplot(2,2,2)
% plot(abs(s_ac_test_row_max_1))
% plot(abs(s_ac_test_row_max))
% [aa1_test,p1_test] = min(abs(s_ac_test_row_max(1,q_test+1:end)));  
% p1_test=p1_test+q_test;
% %x=abs(s_ac_test_row_max(1,q_test:end));
% [aa2_test,p2_test] = max(abs(s_ac_test_row_max(1,p1_test+1:end)));  
% p2_test=p2_test+p1_test+1;
